...
function donutChart () {
  ...
  var variable // value in data that will dictate proportions on chart
  var category // compare data by
  var padAngle // effectively dictates the gap between slices
  var transTime // transition time
  var updateData
  var floatFormat = d3.format('.4r')
  var cornerRadius // sets how rounded the corners are on each slice
  ...
  function chart (selection) {
    selection.each(function () {
      ...
      // ===========================================================================================
      // add text labels
      var label = svg.select('.labelName').selectAll('text') // eslint-disable-line no-unused-vars
        .data(pie(data))
        .enter().append('text')
        .attr('dy', '.35em')
      ...
      var polyline = svg.select('.lines') // eslint-disable-line no-unused-vars
        .selectAll('polyline')
        .data(pie(data))
      ...
      updateData = function () {
        selection.each(function () {
          ...
          var updatePath = svg.select('.slices').selectAll('path')
          var updateLines = svg.select('.lines').selectAll('polyline')
          var updateLabels = svg.select('.labelName').selectAll('text')
      ...
      function labelTween (d) {
        ...
        return function (t) {
          var d2 = interpolate(t)
          var pos = outerArc.centroid(d2) // computes the midpoint [x,y] of the centre line that would be
  ...
  chart.margin = function (value) {
    ...
    margin = extend(defaults.margin, value)
    return chart
  }
